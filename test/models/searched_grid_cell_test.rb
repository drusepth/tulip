...
class SearchedGridCellTest < ActiveSupport::TestCase
  ...
  test "fresh_cache? returns false for stale grid" do
    grid_key = "coffee:45.52:-122.67"
    SearchedGridCell.create!(grid_key: grid_key, category: "coffee", searched_at: (SearchedGridCell::STALE_AFTER + 1.day).ago)
  ...
  test "stale? returns true when searched_at is older than STALE_AFTER" do
    cell = SearchedGridCell.new(searched_at: (SearchedGridCell::STALE_AFTER + 1.day).ago)
    assert cell.stale?
  end
  ...
  test "fresh scope returns only fresh records" do
    ...
    SearchedGridCell.create!(grid_key: stale_key, category: "coffee", searched_at: (SearchedGridCell::STALE_AFTER + 1.day).ago)

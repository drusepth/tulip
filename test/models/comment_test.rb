...
class CommentTest < ActiveSupport::TestCase
  ...
  test "valid comment" do
    comment = Comment.new(stay: @stay, user: @user, body: "Great trip!")
    assert comment.valid?
  end

  test "requires body" do
    comment = Comment.new(stay: @stay, user: @user, body: "")
    assert_not comment.valid?
    assert_includes comment.errors[:body], "can't be blank"
  end

  test "requires stay" do
    comment = Comment.new(user: @user, body: "Test")
    assert_not comment.valid?
    assert_includes comment.errors[:stay], "must exist"
  end

  test "requires user" do
    comment = Comment.new(stay: @stay, body: "Test")
    assert_not comment.valid?
    assert_includes comment.errors[:user], "must exist"
  end

  test "body max length is 2000 characters" do
    comment = Comment.new(stay: @stay, user: @user, body: "a" * 2001)
    assert_not comment.valid?
    assert_includes comment.errors[:body], "is too long (maximum is 2000 characters)"
  end

  test "can have a parent comment" do
    parent = Comment.create!(stay: @stay, user: @user, body: "Parent comment")
    reply = Comment.new(stay: @stay, user: @user, body: "Reply", parent: parent)
    assert reply.valid?
  end

  test "parent must be top-level" do
    parent = Comment.create!(stay: @stay, user: @user, body: "Parent comment")
    reply = Comment.create!(stay: @stay, user: @user, body: "Reply", parent: parent)
    nested = Comment.new(stay: @stay, user: @user, body: "Nested reply", parent: reply)
  ...
  test "top_level scope returns comments without parent" do
    parent = Comment.create!(stay: @stay, user: @user, body: "Parent")
    reply = Comment.create!(stay: @stay, user: @user, body: "Reply", parent: parent)
  ...
  test "ordered scope orders by created_at ascending" do
    first = Comment.create!(stay: @stay, user: @user, body: "First")
    sleep 0.01 # Small delay to ensure different timestamps
    second = Comment.create!(stay: @stay, user: @user, body: "Second")
  ...
  test "editable_by? returns true for author" do
    comment = Comment.create!(stay: @stay, user: @user, body: "Test")
  ...
  test "replies are destroyed when parent is destroyed" do
    parent = Comment.create!(stay: @stay, user: @user, body: "Parent")
    Comment.create!(stay: @stay, user: @user, body: "Reply", parent: parent)
